...
printLM <- function(model)
{
	...
	{
		str1=paste(str1,' +',model$coeff[[i]],'*x',i-1,sep='')
	}
	cat("\n")
	cat("'rmse1=",rmse1,"\n")	
	cat(str1,"\n")
...
i=0
while(i<250){
net.sum <- neuralnet(z~x1+x2+x3,data=MyData,hidden=2,act.fct="logistic");
...
i=0
while(i<250){
net.sum <- neuralnet(z~x1+x2+x3+x4,data=MyData,hidden=5,act.fct="logistic");
...
i=0
while(i<250){
net.sum <- neuralnet(z~x1+x2+x3,data=MyData,hidden=5,act.fct="logistic");
pr.nn <- compute(net.sum,MyData[,1:3]);
if(rmsee(MyData$z-pr.nn[[2]])<kkk){
#print(rmsee(MyData$z-pr.nn[[2]]))
print(Sys.time())
